...
def fill_plane(plane):
    ...
    for name in economy_sold.keys():
        for i in reversed(range(economy_sold[name])): 
    
    # I used the reverse function to reverse the seating order for the economy class passengers being seated so passengers will be assigned seats from aft to forward to ease congestion when boarding the plane.
